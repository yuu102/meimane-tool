...
import { createDefaultDailies, normalizeDailies } from "./dailies.js";
...
export function loadCharacters() {
  for (const key of [CHARACTER_STORAGE_KEY, ...LEGACY_CHARACTER_KEYS]) {
    ...
    characters = parsed.filter((item) => item && typeof item.name === "string").map((item) => {
      ...
      used.add(character.id); return character;
    });
    saveCharacters(); return characters;
  }
  characters = []; return characters;
}

export function replaceCharacters(records) {
  const used = new Set();
  characters = records.map(normalizeCharacter).map((character) => {
    if (used.has(character.id)) character.id = createId();
    used.add(character.id); return character;
  });
  saveCharacters();
