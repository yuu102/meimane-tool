...
- `hideCompleted`: `true` の場合、完了キャラを一覧から非表示にします。総数・完了・残りのサマリーは常に全キャラを対象にします。

自動リセット時はデイリー完了状態を解除し、キャラクターの完了状態を再計算して `meimane.characters` へ保存します。
OFFからONへ変更した当日はリセットせず、その日を `lastResetDate` として保存します。翌日以降の最初の起動から自動リセットを開始します。

バックアップと復元、並び替え、完了キャラの表示切替は設定画面から操作します。
